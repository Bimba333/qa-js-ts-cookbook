...
```text
fixtures    -> подготовка окружения и test data
Page Objects -> действия и чтение UI
assertions  -> проверка ожидаемого поведения
helpers     -> общие операции без знания всего сценария
reporting   -> сбор и форматирование результата
```
