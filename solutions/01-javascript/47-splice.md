...
Распространённая ошибка: ожидать updated source array как return value.
...
Объяснение: `splice()` выполняет middle modification in place. Array нужно читать как mutable state object: до вызова у него одно состояние, после вызова — другое.
...
Объяснение: операция не создает независимый список. Она берет current state array и переводит его в next state. Любая следующая операция работает уже с этим новым состоянием.
...
Объяснение: каждая операция явно меняет середину array и сохраняет removed values. Важно, что третий `splice()` использует index `3` уже после вставки `apply discount`, а не по исходному состоянию array.
