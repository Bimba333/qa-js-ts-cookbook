...
Объяснение: вывод одного step становится вход следующего step.
...
Ответ: как последовательность состояния.
...
Распространённая ошибка: фильтровать по поля, которые исчезли после `map()`.
...
Ответ: когда intermediate состояние важен для понимания.
...
Распространённая ошибка: поменять data shape до того, как все нужные поля использованы.
