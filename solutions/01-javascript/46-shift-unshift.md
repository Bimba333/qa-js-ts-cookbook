...
Array хранит одно value на каждой упорядоченной позиции. Если новое value занимает начало, предыдущие значения получают новые indexes.
...
`shift()` удаляет первый элемент, поэтому в collection становится меньше значения.
...
`Login` становится новым первый элемент. Существующие значения смещаются на одну позицию вправо.
