...
```mermaid
flowchart TD
    A["forEach(колбэк)"] --> B["колбэк на каждый элемент"]
    B --> C["результат колбэка отбрасывается"]
    C --> D["сам метод возвращает undefined"]
    E["break или return"] --> F["обход не прерывают"]
    G["асинхронный колбэк"] --> H["forEach его не дождётся"]
```
