...
```text
текущий код завершился
│
▼
готовы разные отложенные задачи
│
▼
какая выполнится первой?
```
...
```text
Call Stack выполняет текущий код
│
▼
текущий код завершился
│
▼
Event Loop смотрит на microtask queue
│
▼
выполняются Microtasks
│
▼
после этого можно перейти к Macrotasks
```
...
│
▼
высокий приоритет после текущего кода
```
...
│
▼
Event Loop продолжает выполнение
```
...
│
▼
сохранить результат в Microtask
│
▼
позже сгенерировать отчет через таймер
```
