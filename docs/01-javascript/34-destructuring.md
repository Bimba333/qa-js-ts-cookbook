...
Шаблон слева описывает, что достать из значения справа:

```mermaid
flowchart TD
    A["объект или массив"] --> B["шаблон слева от ="]
    B --> C["совпадение по имени свойства<br/>или по позиции"]
    C --> D{"значение найдено?"}
    D -- "нет" --> E["undefined или значение<br/>по умолчанию"]
    D -- "да" --> F["переменная получает значение"]
    G["исходное значение"] --> H["не изменяется"]
```
