...
Остаточный параметр забирает то, что не разобрали именованные:

```mermaid
flowchart TD
    A["вызов с любым числом аргументов"] --> B["именованные параметры<br/>берут свои позиции"]
    B --> C["...rest собирает остальные"]
    C --> D["настоящий массив"]
    E["аргументов не осталось"] --> F["пустой массив, а не undefined"]
```
