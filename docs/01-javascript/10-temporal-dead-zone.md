...
Имя уже есть, но обращаться к нему ещё нельзя:

```mermaid
flowchart TD
    A["вход в область видимости"] --> B["имя зарегистрировано,<br/>но не инициализировано"]
    B --> C["Temporal Dead Zone"]
    C --> D["обращение к имени"]
    D --> E["ReferenceError"]
    C --> F["выполнение дошло до объявления"]
    F --> G["имя инициализировано"]
    G --> H["обращение разрешено"]
```
