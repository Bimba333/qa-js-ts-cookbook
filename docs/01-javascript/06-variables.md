...
Жизненный путь переменной:

```mermaid
flowchart TD
    A["объявление: имя появляется"] --> B{"есть инициализация?"}
    B -- "нет" --> C["значение undefined"]
    B -- "да" --> D["значение связано с именем"]
    C --> E["присваивание"]
    D --> E
    E --> F{"объявлено через const?"}
    F -- "да" --> G["переприсваивание запрещено"]
    F -- "нет" --> H["переприсваивание разрешено"]
```
