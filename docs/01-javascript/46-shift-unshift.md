...
Работа с началом дороже работы с концом:

```mermaid
flowchart TD
    A["массив"] --> B["unshift: добавить в начало"]
    A --> C["shift: забрать первый"]
    B --> D["индексы всех элементов сдвигаются"]
    C --> D
    D --> E["стоимость растёт с длиной"]
    F["push и pop"] --> G["индексы остальных не трогают"]
```
