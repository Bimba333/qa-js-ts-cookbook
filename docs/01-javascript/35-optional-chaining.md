...
`?.` останавливает цепочку, а не подавляет ошибку:

```mermaid
flowchart TD
    A["a?.b?.c"] --> B{"a равно null или undefined?"}
    B -- "да" --> C["вся цепочка даёт undefined"]
    B -- "нет" --> D["читается b"]
    D --> E{"b равно null или undefined?"}
    E -- "да" --> C
    E -- "нет" --> F["читается c"]
    G["ошибка в самом значении"] --> H["оператор её не скрывает"]
```
