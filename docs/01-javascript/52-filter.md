...
```mermaid
flowchart TD
    A["исходный массив"] --> B["предикат на каждый элемент"]
    B --> C{"результат истинный?"}
    C -- "да" --> D["элемент попадает в новый массив"]
    C -- "нет" --> E["элемент пропускается"]
    D --> F["это тот же объект, а не копия"]
