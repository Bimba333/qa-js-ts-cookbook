...
Ошибка меняет маршрут выполнения, а не завершает его сразу:

```mermaid
flowchart TD
    A["try: обычное выполнение"] --> B{"возникла ошибка?"}
    B -- "нет" --> C["блок пройден до конца"]
    B -- "да" --> D["выполнение прерывается"]
    D --> E["catch: обработка"]
    C --> F["finally: выполняется всегда"]
    E --> F
    G["подходящего catch нет"] --> H["ошибка уходит выше по стеку"]
```
