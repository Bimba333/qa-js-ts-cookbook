...
```mermaid
flowchart TD
    A["сравнение значений"] --> B{"какой оператор?"}
    B -- "===" --> C["без преобразования типов"]
    B -- "==" --> D["с преобразованием типов"]
    B -- "Object.is" --> E["как ===, но особые NaN и -0"]
    C --> F{"сравниваются объекты?"}
    F -- "да" --> G["сравниваются ссылки"]
    F -- "нет" --> H["сравниваются значения"]
```
