...
`??` реагирует только на отсутствие значения:

```mermaid
flowchart TD
    A["value ?? запасное"] --> B{"value равно null<br/>или undefined?"}
    B -- "да" --> C["берётся запасное значение"]
    B -- "нет" --> D["берётся value"]
    E["0, пустая строка, false"] --> D
    F["оператор ||"] --> G["заменил бы и эти значения"]
```
