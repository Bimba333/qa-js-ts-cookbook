...
```mermaid
flowchart TD
    N1["Есть ли путь к объекту через ссылки?"]
    N2["есть → объект остается"]
    N3["нет → объект может быть собран"]
    N1 --> N2
    N1 --> N3
```
...
flowchart TD
    N1["ссылка удалена"]
    N2["объект стал unreachable"]
    N3["Garbage Collector"]
    N4["память может быть освобождена позже"]
    N1 --> N2
    N2 --> N3
    N3 --> N4
```
...
flowchart TD
    N1["currentReport"]
    N2["null"]
    N3["old report может стать unreachable"]
    N1 --> N2
    N2 --> N3
```
