...
│
▼
JavaScript продолжает текущий код
│
▼
после готовности задача попадает в очередь
│
▼
Event Loop возвращает ее в выполнение
```

Важно:

```text
JavaScript описывает работу
│
▼
среда выполнения выполняет ожидание
│
▼
JavaScript позже выполняет обратный вызов
```
...
│
▼
среда выполнения ожидает завершения операции
│
▼
обратный вызов возвращается в JavaScript
```
