...
flowchart TD
    A["var"] --> B["let и const"]
    C["конкатенация строк"] --> D["шаблонные строки"]
    E["arguments"] --> F["остаточные параметры"]
    G["колбэки"] --> H["Promise и async/await"]
    I["проверки на undefined"] --> J["?. и ??"]
    K["новый синтаксис"] --> L["не меняет модель выполнения<br/>и не отменяет ошибок"]
```
