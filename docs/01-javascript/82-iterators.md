...
│
▼
получить iterator через Symbol.iterator
│
▼
iterator.next()
│
├── done: false -> взять value
└── done: true  -> остановить цикл
```

Если развернуть обход массива:

```text
next() -> { value: 'login',    done: false }
next() -> { value: 'checkout', done: false }
next() -> { value: 'report',   done: false }
...
{ value, done }
│
├── done: false -> продолжить итерацию
└── done: true  -> остановить итерацию
```
