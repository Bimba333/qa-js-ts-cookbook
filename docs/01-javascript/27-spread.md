...
Одинаковый синтаксис делает противоположные вещи:

```mermaid
flowchart TD
    A["... в вызове или литерале"] --> B["значение разворачивается"]
    B --> C["копия массива или объекта"]
    C --> D["копия поверхностная"]
    D --> E["вложенные объекты остаются общими"]
    F["... в списке параметров"] --> G["это rest: сбор, а не разворот"]
```
