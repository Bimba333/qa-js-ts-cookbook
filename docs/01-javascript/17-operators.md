...
Оператор получает операнды и порождает новое значение:

```mermaid
flowchart TD
    A["операнды"] --> B["оператор"]
    B --> C["результат — новое значение"]
    B --> D["унарный: один операнд"]
    B --> E["бинарный: два операнда"]
    B --> F["тернарный: условие и две ветки"]
    C --> G["операнды не меняются,<br/>если оператор не присваивает"]
```
