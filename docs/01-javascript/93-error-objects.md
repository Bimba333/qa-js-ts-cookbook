...
flowchart TD
    N1["неверные данные"]
    N2["сломанная конфигурация"]
    N3["неожиданный ответ API"]
    N4["ошибка выполнения"]
    N1 --> N2
    N2 --> N3
    N3 --> N4
```
...
```mermaid
flowchart TD
    N1["ошибка — это не просто текст"]
    N2["это объект с данными о проблеме"]
    N3["его можно передать через throw"]
    N4["и осмысленно обработать в catch"]
    N1 --> N2
    N2 --> N3
    N3 --> N4
```
