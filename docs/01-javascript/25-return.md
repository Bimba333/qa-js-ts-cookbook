...
`return` завершает функцию, а не только отдаёт значение:

```mermaid
flowchart TD
    A["тело функции"] --> B{"встретился return?"}
    B -- "да" --> C["выполнение прекращается"]
    C --> D["значение уходит в точку вызова"]
    B -- "нет" --> E["конец тела"]
    E --> F["возвращается undefined"]
```
