...
```mermaid
flowchart TD
    N1["срок действия токена"]
    N2["дата создания заказа"]
    N3["время генерации отчета"]
    N4["длительность теста"]
    N1 --> N2
    N2 --> N3
    N3 --> N4
```
...
flowchart TD
    N1["момент времени"]
    N2["число для сравнения"]
    N3["Date object для методов"]
    N4["строка для вывода"]
    N1 --> N2
    N2 --> N3
    N3 --> N4
```
