...
```mermaid
flowchart TD
    A["Date"] --> B["внутри: миллисекунды от начала эпохи"]
    B --> C["getFullYear, getHours:<br/>локальное время"]
    B --> D["toISOString:<br/>время в UTC"]
    E["месяцы считаются с нуля"] --> F["январь — это 0"]
    G["неразобранная строка"] --> H["Invalid Date без исключения"]
```
