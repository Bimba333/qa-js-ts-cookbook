...
`super` вызывает базовую реализацию с тем же `this`:

```mermaid
flowchart TD
    A["переопределённый метод"] --> B{"есть super.method()?"}
    B -- "нет" --> C["базовая реализация не выполнится"]
    B -- "да" --> D["вызов базового метода"]
    D --> E["this остаётся прежним экземпляром"]
    F["конструктор наследника"] --> G["super() до обращения к this"]
```
