...
Команда `npx playwright install --with-deps chromium` устанавливает браузер, соответствующий версии Playwright из lockfile, и необходимые системные зависимости Linux. Headless mode меняет способ запуска, но не отменяет необходимость в исполняемом файле браузера и системных библиотеках.

Cache браузеров допустим как оптимизация, но ключ должен учитывать ОС, версию Playwright и lockfile. Промах cache обязан приводить к обычной корректной установке. Сбой запуска браузера до выполнения assertions является признаком проблемы инфраструктуры или конфигурации, а не доказательством flaky test.
