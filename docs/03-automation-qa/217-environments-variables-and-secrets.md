...
```mermaid
flowchart TD
    A["переменные окружения"] --> B["недоверенный вход"]
    B --> C{"значение задано?"}
    C -- "нет" --> D["запуск остановлен<br/>с понятной причиной"]
    C -- "пустая строка" --> D
    C -- "да" --> E["конфигурация проверена"]
    F["секрет"] --> G["не возвращается в диагностике"]
```
