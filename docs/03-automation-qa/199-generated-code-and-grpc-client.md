...
Сгенерированный входной тип message используется для подготовки request. Выходной тип описывает десериализованный response. Service client предоставляет методы `GetWorkItem()`, `SearchWorkItems()`, `TransitionWorkItem()` и другие RPC methods контракта.

Client открывает channel к endpoint. Создатель client отвечает за его закрытие. `client.close()` освобождает channel; жизненный цикл самого стенда тестом не управляется.
