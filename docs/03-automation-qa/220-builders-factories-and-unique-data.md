...
```mermaid
flowchart TD
    A["factory: валидная заготовка"] --> B["overrides: только важное сценарию"]
    B --> C["новый объект на каждый вызов"]
    C --> D["уникальность: счётчик или<br/>идентификатор прогона, не время"]
    D --> E["по значению видно,<br/>чей это объект"]
```
