...
URL включает схему, узел, необязательный порт, путь и строку запроса. Параметр пути подставляется в путь, например `/api/v1/work-items/<id>`. Параметр строки запроса уточняет выборку или режим операции, например `?status=NEW&limit=5`. Значения параметров должны кодироваться как данные, а не собираться небезопасной конкатенацией.
