...
```mermaid
flowchart TD
    A["ожидаемое событие"] --> B{"обработчик установлен заранее?"}
    B -- "нет" --> C["страница ждёт<br/>или событие потеряно"]
    B -- "да" --> D["dialog: accept или dismiss"]
    B -- "да" --> E["download: сохранение по пути"]
    E --> F["у файла есть владелец очистки"]
```
