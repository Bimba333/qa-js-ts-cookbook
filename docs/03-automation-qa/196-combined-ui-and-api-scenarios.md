...
Playwright Test может предоставить `page` и `request` одному тесту. Эти fixtures имеют разные клиентские состояния, но обращаются к одному локальному серверу. Серверное состояние связывается только явным ID ресурса.
