...
```mermaid
flowchart TD
    A["builder: форма входных данных"] --> B["build(): новая копия"]
    B --> C["создание ресурса через API"]
    C --> D["тест владеет идентификатором"]
    D --> E["очистка созданного состояния"]
    F["builder не ходит в сеть"] --> A
```
