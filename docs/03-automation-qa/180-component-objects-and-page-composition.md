...
```mermaid
flowchart TD
    A["Page"] --> B["корневой Locator компонента"]
    B --> C["поиск идёт только внутри области"]
    C --> D["тот же компонент работает<br/>в другом месте страницы"]
    A --> E["страница — композиция компонентов"]
```
