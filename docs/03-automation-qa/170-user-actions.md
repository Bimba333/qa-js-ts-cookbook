...
```mermaid
flowchart TD
    A["click()"] --> B["элемент найден"]
    B --> C{"виден, стабилен, доступен,<br/>принимает события?"}
    C -- "нет" --> D["повтор до таймаута"]
    C -- "да" --> E["событие отправлено"]
    D --> F["force: true пропускает проверки,<br/>но не делает элемент готовым"]
```
