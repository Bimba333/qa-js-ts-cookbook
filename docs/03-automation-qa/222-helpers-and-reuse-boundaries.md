...
```mermaid
flowchart TD
    A["вспомогательная функция"] --> B{"что она делает?"}
    B -- "вычисляет" --> C["чистая: вход — выход"]
    B -- "действует" --> D["эффект виден в имени и сигнатуре"]
    E["повторение кода"] --> F["ещё не основание для абстракции"]
    G["флаги и длинный список параметров"] --> H["признак склеенных задач"]
```
