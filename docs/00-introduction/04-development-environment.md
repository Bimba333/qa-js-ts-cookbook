...
Окружение готово
│
├── файл создается в нужной папке
├── команда запускается предсказуемо
├── результат виден в терминале
├── ошибка читается как сообщение
...
│
▼
Node.js выполняет файл
│
▼
результат появляется в терминале
```
...
Текущая папка
│
├── команда ищет файл здесь
│
└── если файла нет, будет ошибка пути
```
...
├── какой файл участвовал
├── какая строка указана
├── какой тип проблемы назван
...
│
▼
Shell получает команду
│
▼
Shell находит программу node
│
▼
Node.js получает путь к файлу
│
▼
Node.js читает файл
│
...
│
▼
Результат выводится в terminal
```
...
node не найден
│
└── Node.js не установлен или недоступен в PATH
...
ошибка в коде
│
└── Node.js смог открыть файл, но не смог выполнить содержимое
```
...
├── practice/: задания
└── solutions/: проверка рассуждений
```
...
```text
Сразу создать большой пример
│
▼
Получить ошибку
│
▼
Не понимать, проблема в окружении или в коде
```
...
```text
Прочитать первую важную строку
│
▼
Найти имя файла
│
...
│
▼
Сразу открыть solutions
│
▼
Выполнить задание по ответу
```
...
│
▼
Запустить через node
│
▼
Сравнить ожидание и факт
│
▼
Удалить или оставить файл как временную заметку
```
...
```text
Локальный запуск hello.js
│
▼
Запуск учебного примера
│
▼
Запуск practice-задания
│
▼
Запуск Playwright-теста
│
▼
Запуск набора тестов в CI
```
...
└── только потом менять тестовый код
```
