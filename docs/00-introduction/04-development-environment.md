...
**Миф: настройка окружения — формальность.**
Половина проблем новичка — это не язык, а несовпадение версий и путей.

**Миф: версия Node.js не важна.**
Разные версии дают разный набор возможностей и разное поведение инструментов.

**Миф: редактор можно выбрать любой, это не влияет на обучение.**
Влияет: подсветка ошибок и переход к определению ускоряют разбор чужого кода.

**Миф: если команда запустилась один раз, окружение настроено.**
Настроенным оно считается тогда, когда запускается воспроизводимо — после перезапуска и на другой машине.

**Миф: глобальная установка пакетов удобнее локальной.**
Она делает проект зависимым от состояния машины, и на другом компьютере всё ломается.

## Частые вопросы

**Что делать, если команда не найдена?**
Проверить установку и переменные среды: чаще всего дело в них, а не в самой команде.

**Зачем фиксировать версии зависимостей?**
Чтобы результат запуска не зависел от дня установки.

**Нужно ли настраивать всё сразу?**
Достаточно запускать примеры. Остальное добавляется по мере необходимости.

**Как убедиться, что окружение готово?**
Запустить пример из первой главы и получить ожидаемый вывод.

**Что делать при расхождении с инструкцией?**
Сначала сверить версии, затем пути. Эти две причины покрывают большинство случаев.
