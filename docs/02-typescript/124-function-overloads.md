...
```mermaid
flowchart TD
    A["вызов функции"] --> B["сигнатура 1"]
    B -- "не подошла" --> C["сигнатура 2"]
    C -- "не подошла" --> D["ошибка вызова"]
    B -- "подошла" --> E["одна общая реализация"]
    C -- "подошла" --> E
    E --> F["реализация обязана<br/>покрывать все варианты"]
```
