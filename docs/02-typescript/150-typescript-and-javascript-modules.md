...
```text
examples/02-typescript/chapter-150/
```

`01-import-export.ts` — именованный импорт из соседнего модуля.

`02-default-export.ts` — импорт значения по умолчанию.

`03-type-as-value-diagnostic.ts` — интерфейс в позиции значения (намеренная ошибка).

`report-line.ts` — модуль с именованными экспортами.

`report-name.ts` — модуль с экспортом по умолчанию.
