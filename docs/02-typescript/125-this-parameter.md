...
```mermaid
flowchart TD
    A["метод объекта"] --> B["вызов obj.method():<br/>this — сам объект"]
    A --> C["метод передан как значение"]
    C --> D["вызов без получателя:<br/>this потерян"]
    D --> E["this-параметр делает это<br/>ошибкой компиляции"]
    E --> F["во время выполнения<br/>параметра не существует"]
```
