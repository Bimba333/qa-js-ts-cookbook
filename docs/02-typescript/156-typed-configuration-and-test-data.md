...
```text
examples/02-typescript/chapter-156/
```

`01-environment-config.ts` — карта окружений через `Record`.

`02-test-data-model.ts` — модель тестового пользователя с `readonly`.

`03-invalid-config-diagnostic.ts` — окружение с пропущенным полем (намеренная ошибка).
