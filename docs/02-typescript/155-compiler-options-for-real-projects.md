...
```text
examples/02-typescript/chapter-155/
```

`01-compiler-options-model.ts` — модель значимых опций компилятора.

`02-lib-does-not-polyfill.ts` — `lib` описывает API, но не добавляет его.

`03-strict-option-diagnostic.ts` — неполная конфигурация проекта (намеренная ошибка).
