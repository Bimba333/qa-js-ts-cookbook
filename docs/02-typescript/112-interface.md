...
```mermaid
flowchart TD
    A["interface LoginPage"] --> B["extends:<br/>расширение договора"]
    A --> C["implements:<br/>класс обещает форму"]
    A --> D["повторное объявление:<br/>слияние деклараций"]
    A --> E["после компиляции:<br/>исчезает полностью"]
```
