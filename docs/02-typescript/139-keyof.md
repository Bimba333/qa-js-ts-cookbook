...
```text
examples/02-typescript/chapter-139/
```

`01-keyof-fields.ts` — `keyof` как объединение имён полей.

`02-locator-map.ts` — карта локаторов и доступ по ключу.

`03-keyof-diagnostic.ts` — имя, которого нет среди ключей (намеренная ошибка).
