...
> **СТАТУС РЕАЛИЗАЦИИ (обновлено 2026-09-21)**
>
> Ограничение `IMPLEMENTATION LIMITED TO SUT-0 PHASE 1` снято решением
> владельца курса. Реализованы и проверены REST, server-rendered UI, unary
> gRPC и наполненная база: `npm run sut:up` поднимает полный стенд,
> `npm run sut:smoke` проходит 23/23 проверки по всем слоям, а
> `npm run sut:test:db` — 47/47 интеграционных тестов.
>
> Отличия реализации от текста ниже, принятые осознанно:
>
> - `test_runs`, `auth_tokens`, `ui_sessions` и `audit_events` добавлены
>   миграциями 004–007; работа с ними описана в `sut/README.md`;
> - миграция 007 расширила чтение для `sut_app` (`password_salt`,
>   `password_hash`, `is_seed`): без них невозможны аутентификация и
>   предсказуемый отказ на seed-записи. Права остались колоночными;
> - timestamps в gRPC передаются строкой ISO 8601, а не well-known type;
> - заголовок `x-creator-test-id` необязателен: без него используется
>   значение `manual`, чтобы ручное изучение API не требовало заголовка;
> - HTTP-слой реализован на `node:http` вместо Fastify — меньше
>   зависимостей в учебном образе;
> - вместо `SELECT ... FOR UPDATE` используется условный `UPDATE` по версии:
>   строчная блокировка потребовала бы табличного права `UPDATE` и разрушила
>   колоночную модель привилегий.
>
> Актуальное описание стенда — `sut/README.md`.
